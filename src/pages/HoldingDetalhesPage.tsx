...
const HoldingDetalhesPage = () => {
  ...
  const holdingData = {
    ...
    etapa: id === "1" ? "concluido" : "documentacao",
  ...
  // Calcula o progresso total com base nos estágios concluídos
  const calcularProgresso = () => {
    const totalEtapas = holdingData.timeline.length;
    const etapasConcluidas = holdingData.timeline.filter(etapa => etapa.completo).length;
    return (etapasConcluidas / totalEtapas) * 100;
  };
  
  // Retorna a etapa atual com base na última etapa concluída
  const getEtapaAtual = () => {
    const etapasConcluidas = holdingData.timeline.filter(etapa => etapa.completo);
    const ultimaEtapaConcluida = etapasConcluidas[etapasConcluidas.length - 1];
    
    if (etapasConcluidas.length === holdingData.timeline.length) {
      return "Concluído";
    }
    
    const indexProximaEtapa = holdingData.timeline.findIndex(etapa => etapa.stage === ultimaEtapaConcluida?.stage) + 1;
    if (indexProximaEtapa < holdingData.timeline.length) {
      return `Em ${holdingData.timeline[indexProximaEtapa].nome}`;
    }
        ...
        <CardContent className="p-0">
          <div className="bg-gradient-to-r from-w1-teal to-w1-mint p-4">
            ...
            <div className="relative">
              <div className="absolute left-0 top-1/3 w-full h-0.5 bg-white/30" />
              
              <div className="flex justify-between relative">
                {holdingData.timeline.map((etapa, index) => (
                  <div 
                    key={etapa.stage}
